...
def initfit_new():
	...
	mod_params_temp = np.zeros((nparams-1,numDataPerRank))
	...
	for ii in recvbuf_idx:
		...
		photo_SED = generate_modelSED_photo_fit(sp=sp,imf_type=imf,sfh_form=sfh_form,filters=filters,
																	add_igm_absorption=add_igm_absorption,igm_type=igm_type,
																	params_fsps=params_fsps, params_val=params_val,DL_Gpc=DL_Gpc,
																	cosmo=cosmo_str,H0=H0,Om0=Om0,free_z=free_z,trans_fltr_int=trans_fltr_int)
	...
	mod_params = np.zeros((nparams-1,numDataPerRank*size))
	...
	if rank == 0:
		...
		lowest_chi2[0] = mod_chi2[idx0]
		print ("Lowest reduced chi2 in the initial fitting: %lf" % (lowest_chi2[0]/nbands))

		# parameters of best-fit model
		for pp in range(0,nparams-1):
			params_chi2min_initfit[pp] = mod_params[pp][idx0]

		if mod_chi2[idx0]/nbands > redcd_chi2:  
			sys_err_frac = 0.01
			while sys_err_frac <= 0.5:
				# modifiy flux uncertainty:
			...
			norm_temp = model_leastnorm(obs_fluxes,modif_obs_flux_err,fluxes)
		else:
	...
	comm.Bcast(lowest_chi2, root=0)

	return lowest_chi2,params_chi2min_initfit,modif_obs_flux_err


def initfit(increase_ferr,mod_params0,mod_fluxes0):
	...
	if increase_ferr == 1:
		...
		mod_fluxes = np.transpose(mod_fluxes0, axes=(1,0))     ## become [idx-model][idx-band] 
		...
		if rank == 0:
			...
			params_chi2min_initfit = np.zeros(nparams)  	# include log_norm
			for pp in range(0,nparams-1):
				params_chi2min_initfit[pp] = mod_params0[pp][idx0]
	...
	return params_chi2min_initfit,modif_obs_flux_err,mod_params,mod_fluxes
	...
	if sfh_form == 'tau_sfh' or sfh_form == 'delayed_tau_sfh':
		if duste_switch == 'noduste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					...
					params = ['logzsol','log_tau','log_age','dust2','log_mass']     # nparams = 5
					nparams = len(params)
				elif add_agn == 1:
					# fix-z, mainSFH, noduste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_age','dust2','log_fagn','log_tauagn','log_mass']     # nparams = 5
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# fix-z, mainSFH, noduste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_age','dust1','dust2','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, noduste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_age','dust1','dust2','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# fix-z, mainSFH, noduste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_age','dust_index','dust1','dust2','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, noduste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_age','dust_index','dust1','dust2','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
		elif duste_switch == 'duste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					# fix-z, mainSFH, duste, Cal2000, noAGN:
					params = ['logzsol','log_tau','log_age','dust2','log_gamma','log_umin','log_qpah','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# fix-z, mainSFH, duste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_age','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# fix-z, mainSFH, duste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, duste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# fix-z, mainSFH, duste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, duste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
		...
		if duste_switch == 'noduste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					...
					params = ['logzsol','log_tau','log_t0','log_age','dust2','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# fix-z, otherSFH, noduste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_t0','log_age','dust2','log_fagn','log_tauagn','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# fix-z, mainSFH, noduste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust1','dust2','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, noduste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust1','dust2','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# fix-z, mainSFH, noduste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust_index','dust1','dust2','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, noduste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust_index','dust1','dust2','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
		elif duste_switch == 'duste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					# fix-z, mainSFH, duste, Cal2000, noAGN:
					params = ['logzsol','log_tau','log_t0','log_age','dust2','log_gamma','log_umin','log_qpah','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# fix-z, mainSFH, duste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_t0','log_age','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# fix-z, mainSFH, duste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, duste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# fix-z, mainSFH, duste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, duste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
		...
		if duste_switch == 'noduste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					# fix-z, mainSFH, noduste, Cal2000, noAGN:
					params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust2','log_mass']     # nparams = 5
					nparams = len(params)
				elif add_agn == 1:
					# fix-z, mainSFH, noduste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust2','log_fagn','log_tauagn','log_mass']     # nparams = 5
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# fix-z, mainSFH, noduste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust1','dust2','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, noduste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust1','dust2','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# fix-z, mainSFH, noduste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust_index','dust1','dust2','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, noduste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust_index','dust1','dust2','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
		elif duste_switch == 'duste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					# fix-z, mainSFH, duste, Cal2000, noAGN:
					params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust2','log_gamma','log_umin','log_qpah','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# fix-z, mainSFH, duste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# fix-z, mainSFH, duste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, duste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# fix-z, mainSFH, duste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# fix-z, mainSFH, duste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','log_mass']
						nparams = len(params)
	...
	if sfh_form == 'tau_sfh' or sfh_form == 'delayed_tau_sfh':
		if duste_switch == 'noduste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					...
					params = ['logzsol','log_tau','log_age','dust2','z','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# free-z, mainSFH, noduste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_age','dust2','log_fagn','log_tauagn','z','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# free-z, mainSFH, noduste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_age','dust1','dust2','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, noduste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_age','dust1','dust2','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# free-z, mainSFH, noduste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_age','dust_index','dust1','dust2','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, noduste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_age','dust_index','dust1','dust2','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
		elif duste_switch == 'duste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					# free-z, mainSFH, duste, Cal2000, noAGN:
					params = ['logzsol','log_tau','log_age','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# free-z, mainSFH, duste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_age','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# free-z, mainSFH, duste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, duste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# free-z, mainSFH, duste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, duste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
		...
		if duste_switch == 'noduste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					...
					params = ['logzsol','log_tau','log_t0','log_age','dust2','z','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# free-z otherSFH, noduste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_t0','log_age','dust2','log_fagn','log_tauagn','z','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# free-z, mainSFH, noduste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust1','dust2','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, noduste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust1','dust2','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# free-z, mainSFH, noduste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust_index','dust1','dust2','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, noduste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust_index','dust1','dust2','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
		elif duste_switch == 'duste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					# free-z, mainSFH, duste, Cal2000, noAGN:
					params = ['logzsol','log_tau','log_t0','log_age','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# free-z, mainSFH, duste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_t0','log_age','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# free-z, mainSFH, duste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, duste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# free-z, mainSFH, duste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, duste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_t0','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']       ### 14 parameters
						nparams = len(params)
		...
		if duste_switch == 'noduste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					# free-z, mainSFH, noduste, Cal2000, noAGN:
					params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust2','z','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# free-z, mainSFH, noduste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust2','log_fagn','log_tauagn','z','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# free-z, mainSFH, noduste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust1','dust2','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, noduste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust1','dust2','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# free-z, mainSFH, noduste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust_index','dust1','dust2','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, noduste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust_index','dust1','dust2','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
		elif duste_switch == 'duste':
			if dust_ext_law == 'Cal2000':
				if add_agn == 0:
					# free-z, mainSFH, duste, Cal2000, noAGN:
					params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
					nparams = len(params)
				elif add_agn == 1:
					# free-z, mainSFH, duste, Cal2000, AGN:
					params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']
					nparams = len(params)
			elif dust_ext_law == 'CF2000':
				if fix_dust_index == 1:
					if add_agn == 0:
						# free-z, mainSFH, duste, CF2000, fix dust_index, noAGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, duste, CF2000, fix dust_index, AGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
				elif fix_dust_index == 0:
					if add_agn == 0:
						# free-z, mainSFH, duste, CF2000, vary dust_index, noAGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','z','log_mass']
						nparams = len(params)
					elif add_agn == 1:
						# free-z, mainSFH, duste, CF2000, vary dust_index, AGN:
						params = ['logzsol','log_tau','log_alpha','log_beta','log_age','dust_index','dust1','dust2','log_gamma','log_umin','log_qpah','log_fagn','log_tauagn','z','log_mass']
						nparams = len(params)
...
mod_params0 = []
mod_fluxes0 = []
params_chi2min_initfit,modif_obs_flux_err,mod_params,mod_fluxes = initfit(increase_ferr,mod_params0,mod_fluxes0)
if np.sum(modif_obs_flux_err) != 0:
	obs_flux_err = modif_obs_flux_err
	increase_ferr = 0
	mod_params0 = mod_params
	mod_fluxes0 = mod_fluxes
	params_chi2min_initfit,modif_obs_flux_err,mod_params,mod_fluxes = initfit(increase_ferr,mod_params0,mod_fluxes0)
