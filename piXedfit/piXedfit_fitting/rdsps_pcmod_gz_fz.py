...
def bayesian_sedfit_gauss():
	...
	for ii in recvbuf_idx:
		...
		norm0 = calc_posteriors.model_leastnorm(obs_fluxes,obs_flux_err,fluxes)
		...
		chi2 = np.sum((mod_fluxes0-obs_fluxes)*(mod_fluxes0-obs_fluxes)/obs_flux_err/obs_flux_err)
		prob0 = calc_posteriors.gauss_prob(obs_fluxes,obs_flux_err,mod_fluxes0)
...
### determine indexes of the grid_z that are goind to be used for interpolation
idx_grid_z0 = -99
idx_grid_z1 = -99
#for ii in range(0,ngrid_z-1):
#	if grid_z[ii] <= gal_z < grid_z[ii+1]:
#		idx_grid_z0 = ii
#		idx_grid_z1 = ii + 1
#		break
idx0 = np.interp(gal_z,grid_z,idx_grid_z_temp)
idx_grid_z0 = int(idx0)
idx_grid_z1 = int(idx0) + 1

if idx_grid_z0 == -99:
	print ("the galaxy's redshift must lie within the range avalable in the random model SEDs fits file")
	sys.exit()
idx_grid_z = []
if idx_grid_z0 == 0:
	for ii in range(0,3):
		idx_grid_z.append(ii)
elif idx_grid_z1 == ngrid_z:
	for ii in range(0,3):
		idx_grid_z.append(ngrid_z-ii)
else:
	for ii in range(idx_grid_z0-1,idx_grid_z1+2):
		idx_grid_z.append()
n_idx_grid_z = len(idx_grid_z)

### make grid of redshifts for the interpolation:
ref_grid_z = np.zeros(n_idx_grid_z)
for ii in range(0,n_idx_grid_z):
	ref_grid_z[ii] = grid_z[int(idx_grid_z[ii])]
