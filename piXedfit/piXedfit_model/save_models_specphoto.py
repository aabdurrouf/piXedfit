...
import piXedfit_spectrophotometric
...
from piXedfit.piXedfit_model import generate_modelSED_spec_fit, generate_modelSED_propspecphoto_nomwage_fit, calc_mw_age
...
# wavelength range of model spectrum
min_spec_wave = float(config_data['min_wave'])
max_spec_wave = float(config_data['max_wave'])
...
# number of model SEDs will be generated
global npmod_seds
...
# Get the wavelength and wavelength ids of sliced wavelength
idx_incld = np.where((spec_SED['wave']>=min_spec_wave) & (spec_SED['wave']<=max_spec_wave))
idx_min_wave = min(idx_incld[0])
idx_max_wave = max(idx_incld[0]) + 1
mod_spec_wave = spec_SED['wave'][idx_min_wave:idx_max_wave]
n_spec_wave = len(mod_spec_wave)

# Generate random models uniformly drawn within the prior ranges
...
mod_spec_flux_temp = np.zeros((n_spec_wave,numDataPerRank))
...
for ii in recvbuf_idx:
	...
	# get model fluxes
	SED_prop,photo_SED,spec_SED = generate_modelSED_propspecphoto_nomwage_fit(sp=sp,imf_type=imf,sfh_form=sfh_form,
												filters=filters,add_igm_absorption=add_igm_absorption,igm_type=igm_type,params_fsps=params_fsps,
												params_val=params_val,DL_Gpc=DL_Gpc,cosmo=cosmo_str,H0=H0,Om0=Om0,free_z=free_z,
												trans_fltr_int=trans_fltr_int)

	# cut the spectrum
	spec_flux_temp0 = spec_SED['flux'][idx_min_wave:idx_max_wave]
	# spectral smoothing
	wave,spec_flux_temp = piXedfit_spectrophotometric.spec_smoothing(mod_spec_wave,spec_flux_temp0,spec_sigma)

	# get photometric fluxes
	...
	for bb in range(0,n_spec_wave):
		mod_spec_flux_temp[bb][int(count)] = spec_flux_temp[bb]
...
mod_spec_flux = np.zeros((n_spec_wave,npmod_seds))
...
# transpose from (idx-wave,idx-mod) ==> (idx-mod,idx-wave)
mod_spec_flux_trans = np.transpose(mod_spec_flux, axes=(1,0))
...
if rank == 0:
	...
	for bb in range(0,nbands):
		...
		cols0.append(col)
	cols = fits.ColDefs(cols0)
	hdul.append(fits.BinTableHDU.from_columns(cols, name='photo_SED'))

	# spectroscopic SED
	cols0 = []
	col = fits.Column(name='wave', format='D', array=np.array(mod_spec_wave))
	cols0.append(col)
	for ii in range(0,npmod_seds):
		col = fits.Column(name='mod-%d' % ii, format='D', array=np.array(mod_spec_flux_trans[ii]))
		cols0.append(col)
	cols = fits.ColDefs(cols0)
	hdul.append(fits.BinTableHDU.from_columns(cols, name='spec_SED'))
